...
const login = async (phone, password)=>{
    try {
        ...
        // 处理云函数返回结果
        const response = typeof result === 'object' && 'result' in result ? result.result : result;
        if (!response.success) {
...
;
// 配置：微信云开发云函数调用
const CLOUD_ENV_ID = 'cloudbase-5gqcz0ab010d3288';
// 云函数调用适配器（如果是在小程序环境中）
const callCloudFunction = async (name, data)=>{
    ...
    // 如果是在 Web 环境，需要通过 HTTP API 调用云函数
    // 需要配置云函数 HTTP 触发 URL
    const HTTP_API_BASE_URL = process.env.NEXT_PUBLIC_CLOUD_API_URL || 'https://your-cloud-api-domain.com';
    const response = await fetch(`${HTTP_API_BASE_URL}/${name}`, {
        method: 'POST',
        headers: {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d6c0e02f._.js.map