...
import { setIsExporting } from "@/store/slices/memeSlice";
...
export function useMemeExport({
    ...
    try {
      ...
      for (const draft of memeDrafts) {
        ...
        if (
          ...
          draft.groupResults &&
          draft.groupResults.length > 1
        ) {
          console.warn(
            `草稿 ${draft.id} 似乎丢失了 prompts 数据，但有 ${draft.groupResults.length} 个生成结果。尝试恢复...`
          );
        ...
        const hasAnimation =
          draft.animation && draft.animation !== AnimationType.NONE;
          ...
          for (let i = 0; i < prompts.length; i++) {
            ...
            if (groupResult?.generatedUrl && groupResult.status === "done") {
              try {
                ...
                const baseFileName = `meme_${memeDrafts.indexOf(draft) + 1}_${
                  i + 1
                }`;
                ...
                const groupTextPosition =
                  prompts[i]?.textPosition || draft.textPosition || "bottom";
                ...
                if (ffmpeg) {
                  try {
                    ...
                    const processedUrl =
                      processedImageRef.current[processedKey];
            ...
            try {
              ...
              if (ffmpeg) {
                try {
                  ...
                  const processedUrl =
                    processedImageRef.current[processedKey] ||
                    processedImageRef.current[draft.id];
  ...
  }, [memeDrafts, getTextPrompts, processedImageRef, initFFmpeg, dispatch]);
